    ...
    <div className="w-full flex">
      <div className="grid grid-row-7 grid-flow-row gap-3">
        {weekDays.map((weekDay, key) => {
          return(
            <div key={key} className="text-zinc-400 text-xl h-10 w-10 font-bold flex items-center justify-center">
              {weekDay}
            </div>
          )
        })}
      ...
      <div className="grid grid-rows-7 grid-flow-col gap-3">
        ...
        {amountOfDaysToFill > 0 && Array.from({ length: amountOfDaysToFill }).map((_, key) => {
          return (
            <div key={key} className="w-10 h-10 bg-zinc-900 border-2 border-zinc-800 rounded-lg opacity-40" />
          )
        })}
      </div>
    </div>
